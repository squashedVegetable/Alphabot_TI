...
# ----------------------------------------------------------------------------
# 1. check for obstacles and, if detected, run digit recognition
# -----------------------------------------------------------------------------
def check_obstacle_and_recognize_digit(bot):
            if bot.infrared_obstacle_check():
                ...
                if digit is not None:
                    ...
                    # Buzz the number of times equal to the recognized digit
                ...
                bot.line_following_active = True

# ----------------------------------------------------------------------------
# 3. Update object recognition with servo adjustments based on a simple state machine.
# ----------------------------------------------------------------------------
def update_object_recognition(bot):
...
if __name__ == '__main__':
    ...
    threadCheckObstacles = threading.Thread(target=check_obstacle_and_recognize_digit, args=(bot,))
    threadLineFollow = threading.Thread(target = bot.update_line_follow, args=())
    threadUpdateObjectREcognition = threading.Thread(target = update_object_recognition, args = (bot,))

    try:
        while not stop_event:
            threadCheckObstacles.start()
            threadLineFollow.start()
            threadUpdateObjectREcognition.start()
